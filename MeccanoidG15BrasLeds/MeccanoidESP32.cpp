/* Pilote local adapte pour ESP32 : temporisation de reception corrigee. */
/*
...
bool Module :: onInput(byte input) {
	// Un servo ne peut renvoyer une position qu'entre ces deux bornes.
	// Ne jamais remplacer la derniere bonne position par 0 ou par un octet de
	// protocole recu au mauvais moment.
	if (type == TYPE_SERVO) {
		if (input < SERVO_MIN || input > SERVO_MAX)
			return false;

		bool accepterPosition = lastInput < SERVO_MIN || lastInput > SERVO_MAX
		                    || abs((int)input - (int)lastInput) <= MAX_POSITION_JUMP_RAW;

		if (!accepterPosition && hasPendingInput) {
			// Deux lectures successives allant dans la meme zone confirment un
			// mouvement rapide. Un aller-retour immediat vers lastInput confirme
			// au contraire que la premiere lecture etait un parasite.
			accepterPosition = abs((int)input - (int)pendingInput)
			                 <= MAX_POSITION_JUMP_RAW;
		}

		if (!accepterPosition) {
			pendingInput = input;
			hasPendingInput = true;
			missedResponses = 0; // la communication est bonne, seule la valeur attend confirmation
			return true;
		}
...
bool Module :: onMissingInput() {
	if (missedResponses < 255)
		missedResponses++;
	return missedResponses >= MAX_MISSED_RESPONSES;
}
...
uint32_t ServoAdapter :: getInputSequence() {
	return module.inputSequence;
}
...
void Chain :: update() {
	...
	// 0xFE ne sert a demander le type que pendant la decouverte. Une lecture
	// parasite a 0xFE sur un module deja connu ne doit pas modifier sa commande.
	if (!module.isConnected() && input == NEWMOD_BYTE)
	...
	if (!module.isConnected()) {
		...
		// Conserver la derniere position valide pendant une perturbation. Dans
		// l'ancien pilote, une seule lecture a 0 effacait la position puis
		// deconnectait immediatement le module, ce qui provoquait les bonds et
		// la longue phase de redetection observee pendant l'enregistrement.
		if (!module.onInput(input) && module.onMissingInput()) {
			module.disconnect();
...
byte Chain :: receiveByte() {
	...
	// ESP32 : delay() tronque 1.5 a 1 ms. Le protocole exige 1500 us exactes.
